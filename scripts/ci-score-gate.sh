...
# NOTE: SCORE is wall-clock ns — only comparable on a fixed reference runner.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "Current record SCORE: $best ns (lower is better)"

if ! bash scripts/evaluate.sh --no-guard 2>&1 | tee /tmp/bootstrap_eval.out; then
  echo "::error::evaluate.sh failed — candidate is INVALID"
  exit 1
fi

score="$(sed -n 's/^SCORE: \([0-9][0-9]*\).*/\1/p' /tmp/bootstrap_eval.out | tail -1)"
if [[ -z "$score" ]]; then
  echo "::error::No numeric SCORE in evaluate output"
  exit 1
fi

echo "Candidate SCORE: $score ns"

if (( score >= best )); then
  echo "::error::SCORE $score does not beat the current record $best (lower is better)"
  echo "::error::(wall-clock is noisy — beat the record by a clear margin on the reference runner)"
  exit 1
fi
