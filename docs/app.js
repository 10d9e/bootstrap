...
// SCORE is wall-clock nanoseconds; show it as human time.
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (item) =>
              `SCORE: ${fmt(CHART_STATE.scored[CHART_STATE.lo + item.dataIndex].score)} ns`,
          },
        },
      },
      scales: {
        ...
        y: {
          ...
          title: {
            display: true,
            text: "wall-clock ns per bootstrap (lower is faster)",
...
async function main() {
  try {
    ...
    const repo = data.repo || "10d9e/bootstrap";
    ...
    if (hashId && ENTRIES_BY_ID[hashId]) openDialog(ENTRIES_BY_ID[hashId], repo);
      ...
      try {
        if (localStorage.getItem("bootstrap-instructions-seen") !== "1") {
          $("#instructionsBtn").click();
          localStorage.setItem("bootstrap-instructions-seen", "1");
        }
      } catch (_) {}
    }
  } catch (err) {
